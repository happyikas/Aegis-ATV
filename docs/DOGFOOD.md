...
The dummy safety classifier's regex caught `rm -rf /` (TP-1), `DROP TABLE` (TP-2),
and the prompt-injection phrase `ignore all previous instructions` (TP-3) in tool arguments.
...
strings as test data (e.g. `rm -rf /` in the `command` field). The Write
...
`printf 'fake test: `rm -rf /`'` → BLOCK. The `printf` is shell-safe, but
...
because the report quotes the trigger strings (`rm -rf /`, `DROP TABLE`, `ignore all previous instructions`).
